...
/* The only modifications to this file from the version in KOS are that it
   mounts itself on /gd instead of /cd, and several cdrom_* calls have been
   replaced by low-level code that is in gd.s. Note that the code in gd.s
   doesn't actually let you read GD-ROMs in and of itself. It is fully locked to
   read CDs only. "How does this code actually work in the patcher to read the
   GD-ROM then?" you might ask. It's a secret to everybody. */
...
/* Blah. */
static uint32 be_733(const void *data) {
    const uint8 *d = (const uint8*)data;
    return (d[7] << 0) | (d[6] << 8) | (d[5] << 16) | (d[4] << 24);
}
...
static uint32 iso_733(const uint8 *from) { return be_733(from); }
...
static int bread_cache(cache_block_t **cache, uint32 sector) {
    ...
    sz = 2048;
    gd_read_sector(sector + 150, (uint16 *)cache[i]->data, &sz);
